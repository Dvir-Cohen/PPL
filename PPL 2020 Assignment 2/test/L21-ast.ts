...
//L21
export const makeForExp = (loop_var: VarDecl, start:NumExp, end:NumExp, body:CExp ): ForExp =>
    ({tag: "ForExp", var: loop_var, start:start, end:end, body:body});
...
const parseForExp = (vars: Sexp, body: Sexp[]): Result<ForExp> =>
body.length === 3 && isIdentifier(vars) 
                    && isString(body[0]) && isNumericString (body[0]) 
                    && isString(body[1]) && isNumericString(body[1])
                    && +body[0]<= +body[1]?
    bind(mapResult(parseL21CExp, body),
        (cexps: CExp[]) => makeOk(makeForExp ( makeVarDecl (vars), <NumExp>cexps[0] , <NumExp>cexps[1] , cexps[2]))):
