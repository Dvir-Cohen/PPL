...
import { isCExp,VarDecl, CExp } from './L4-ast';
...
//for bonus - holds each exp with env
export interface Env_CExp {
    tag: "Env_CExp";
    env: Env;
    exp:CExp;
}
export const makeEnv_CExp = (env:Env, exp:CExp) : Env_CExp=> ({tag:"Env_CExp", env,exp});


export const makeEmptyEnv = (): EmptyEnv => ({tag: "EmptyEnv"});
export const makeExtEnv = (vs: string[], vals: Env_CExp[], env: Env): ExtEnv =>
...
// Apply-env
export const applyEnv = (env: Env, v: string): Result<Env_CExp> =>
    isEmptyEnv(env) ? makeFailure(`var not found ${v}`) :
    isExtEnv(env) ? applyExtEnv(env, v) :
    applyRecEnv(env, v);

const applyExtEnv = (env: ExtEnv, v: string): Result<Env_CExp> =>
    env.vars.includes(v) ? makeOk(env.vals[env.vars.indexOf(v)]) :
    applyEnv(env.nextEnv, v);

//for bonus
const applyRecEnv = (env: RecEnv, v: string): Result<Env_CExp> =>
    env.vars.includes(v) ?  makeOk(makeEnv_CExp(env,env.bodiess[0][env.vars.indexOf(v)])) : 
    applyEnv(env.nextEnv, v);
