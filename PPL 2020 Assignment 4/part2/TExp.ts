...
;; [Empty -> void]
*/
import { chain, concat, map, uniq,contains } from "ramda";
...
import { Result, bind, makeOk, makeFailure, safe2, mapResult, isOk, isOkT, isFailure } from "../shared/result";
import { parse as p } from "../shared/parser";
import { LetValues } from "./L5-ast";
...
//export type otherCompoundTExp = { tag: "otherCompoundTExp"; paramTEs: TExp[]; };
//export const makeOtherCompoundTExp = (paramTEs: TExp[]): otherCompoundTExp =>
  //      ({tag: "otherCompoundTExp", paramTEs: paramTEs});
//export const isotherCompoundTExp = (x: any): x is otherCompoundTExp => x.tag === "otherCompoundTExp";
///export const otherCompoundTExpComponents = (pt: otherCompoundTExp): TExp[] =>
   // [...pt.paramTEs];
...
// export type valuesTExp = { tag: "valuesTExp"; valsTEs: TExp[]; returnTE:TExp };
// export const makevaluesTExp = (valsTEs: TExp[], returnTE: TExp): valuesTExp =>
//     ({tag: "valuesTExp", valsTEs: valsTEs, returnTE: returnTE});
// export const isValuesTExp = (x: any): x is valuesTExp => x.tag === "valuesTExp";

// export type LetValuesTExp = { tag: "let-valuesTExp"; valsTEs: TExp[]; returnTE:TExp };
// export const makeLetValuesTExp = (valsTEs: TExp[], returnTE: TExp): LetValuesTExp =>
//     ({tag: "let-valuesTExp", valsTEs: valsTEs, returnTE: returnTE});
// export const isLetValuesTExp = (x: any): x is valuesTExp => x.tag === "let-valuesTExp";
...
//const parseCompoundTExp = (texps: Sexp[]): Result<ProcTExp> => {
    ...
    const texpArr = parseTupleTExp(texps);
            return texps.includes("Empty"||"empty") && texps.length ===1?  makeOk(makeEmptyTupleTExp()):
            //makeFailure("Empty tuple wrong"):
            (pos === -1)? isFailure(texpArr)? makeFailure("bad type") :makeOk( makeNonEmptyTupleTExp(texpArr.value)):
