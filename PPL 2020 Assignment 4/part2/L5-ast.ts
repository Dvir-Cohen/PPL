...
import { join, map, zipWith, mergeAll, is } from "ramda";
...
export const makeValues = ( val: CExp[]): values =>
    ({tag: "values", val: val});
export const isValuesExp = (x: any): x is values => x.tag === "values";


export interface LetValues {tag: "let-values"; bindings: ValuesBinding[] ;body: CExp[];  }
export const makeLetValues = ( bindings: ValuesBinding[] ,body: CExp[]): LetValues =>
   ({tag: "let-values" ,bindings: bindings, body:body});
export const isLetValuesExp = (x: any): x is LetValues => x.tag === "let-values";

export interface ValuesBinding {tag: "ValuesBinding"; var: VarDecl[]; val: CExp; }
export const makeValuesBinding = (v: VarDecl[], val: CExp): ValuesBinding =>
    ({tag: "ValuesBinding", var: v, val: val});
...
export const parseValues = (params: Sexp[]): Result<values> =>
     bind(mapResult(parseL5CExp, params),
         (cexps: CExp[]) => makeOk(makeValues(cexps)));    
...
export const parseLetValues = (vars: Sexp, rest: Sexp[]): Result<LetValues> =>{
    if (isArray(vars) ){
        const bindings = mapResult(SexpToValuesBindings , vars)
        const body = mapResult(parseL5CExp,rest);
        return safe2((b: ValuesBinding[], body: CExp[]) => makeOk(makeLetValues(b, body)))
        (bindings , body);
    } else {
        return makeFailure(`Invalid  ${JSON.stringify(vars)}`)
    }
  }

  const SexpToValuesBindings=(exp:Sexp):Result<ValuesBinding>=>
    isArray(exp) && isArray(exp[0])?  safe2  ((vars: VarDecl[], vals: CExp) => makeOk(makeValuesBinding( vars, vals)))
                                    ((mapResult(parseVarDecl, isArray(exp[0])? exp[0]: [])), (parseL5CExp(exp[1]))):
    makeFailure("")
...
export const unparse = (e: Parsed): Result<string> =>
    ...
    isValuesExp(e)? makeOk(`(values ${unparseArr((mapResult(unparse, e.val)))})`) :
    isLetValuesExp(e)? makeOk(`(let-values (${(map(unparseLetValues,e.bindings)).join(' ')}) ${unparseArr((mapResult(unparse, e.body)))})`):
    bind(mapResult(unparse, e.exps), (exps: string[]) => makeOk(`(L5 ${exps})`));

const unparseLetValues= (b:ValuesBinding):string=>
    `((${unparseArr((mapResult(unparseVarDecl, b.var)))}) ${unparseRes(unparse(b.val))})`
...
export const unparseArr = (cexps:Result<string[]>): string =>
        isOk(cexps)? cexps.value.join(' '):
        "failure";

export const unparseRes = (cexps:Result<string>): string =>
        isOk(cexps)? cexps.value:
        "failure";
