// L5-typecheck
// ========================================================
import { equals, map, zipWith, is, flatten } from 'ramda';
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         isPrimOp, isProcExp, isProgram, isStrExp, isVarRef, parseL5Exp, unparse,
         AppExp, BoolExp, DefineExp, Exp, IfExp, LetrecExp, LetExp, NumExp,
         Parsed, PrimOp, ProcExp, Program, StrExp, isValuesExp, values, CExp, isLetValuesExp, LetValues, makeNumExp, ValuesBinding, VarDecl, Binding } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, makeNonEmptyTupleTExp, isNonEmptyTupleTExp, NonEmptyTupleTExp, isTupleTExp } from "./TExp";
...
//import { makeTupleValue } from './L5-value';
...
export const L5typeof = (concreteExp: string): Result<string> =>{
    return bind(bind(p(concreteExp), parseL5Exp),
...
export const typeofExp = (exp: Parsed, tenv: TEnv): Result<TExp> =>
    isValuesExp(exp)? (typeofValues(mapResult((e:CExp)=>typeofExp(e,tenv),exp.val))):
    ...
    makeFailure("Unknown type");
     ...
     const typeofValues = (exp:Result<TExp[]>): Result<TExp> =>
        isOk(exp)? makeOk(makeNonEmptyTupleTExp(exp.value)):
        makeFailure ("Unknown type");

     const typeofLetValues = (exp:LetValues, tenv:TEnv):Result<TExp> =>{
        const varsGivanTEs = (map( (v:VarDecl)=> v.texp ,flatten(map((b:ValuesBinding) => b.var, exp.bindings))));
        const vars = (map( (v:VarDecl)=> v.var ,flatten(map((b:ValuesBinding) => b.var, exp.bindings))));
        const TuplesArr = mapResult((b:ValuesBinding)=> typeofExp(b.val,tenv), exp.bindings);
        if (isOk(TuplesArr) ){
           const varEvalTEs = flatRes( mapResult((t:TExp )=>(isNonEmptyTupleTExp(t)? makeOk(t.TEs): makeOk([t])), TuplesArr.value  ))
            if (isOk(varEvalTEs)  && equals(varsGivanTEs,varEvalTEs.value)){
                return typeofExps(exp.body, makeExtendTEnv(vars, varsGivanTEs, tenv));}
        }
        return makeFailure ("Unknown type");
       }

       const flatRes= (r:Result<TExp[][]>):Result<TExp[]>=>
            isOk(r)? makeOk(flatten(r.value)): makeFailure("bad type")
