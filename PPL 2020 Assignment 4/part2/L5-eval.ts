...
import { map, repeat, zipWith,unnest, flatten, compose, transpose } from "ramda";
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, isValuesExp, isLetValuesExp, values, LetValues, makeBinding, makeNumExp, ValuesBinding } from './L5-ast';
...
import { isClosure, makeClosure, Closure, Value, SExpValue, isSExp, makeTuple, isTupleValue } from "./L5-value";
...
//import { LetValuesTExp } from "./TExp";
...
export const applicativeEval = (exp: CExp, env: Env): Result<Value > =>
    ...
    isAppExp(exp) ? safe2((proc: Value, args: SExpValue[]) => applyProcedure(proc, args))
                        ((applicativeEval(exp.rator, env)), mapResult(rand => SExpCheck(applicativeEval(rand, env)), exp.rands)) :
    isValuesExp(exp)?  evalValues(mapResult(val => applicativeEval (val,env) ,exp.val)):
    isLetValuesExp(exp)?  evalLetValues(exp, env) :
    makeFailure(`Bad L5 AST ${exp}`);

    const SExpCheck = (res:Result<Value>):Result<SExpValue>=>
        isOk(res) && isSExp(res.value)? makeOk(res.value):
        makeFailure("APPExp ileagal args");
    
    
export const evalValues= (exp:Result<Value[]>):Result <Value>=>
    isOk(exp)? makeOk(makeTuple(exp.value)):
    exp;


export const evalLetValues = (exp:LetValues, env:Env): Result <Value> =>{
    const val =flat(mapResult((b:ValuesBinding)=> unevalValues(b.var,evalCExps(b.val,[],env)), exp.bindings));
    const vars = map((v:VarDecl)=> v.var ,(flatten( map((b: ValuesBinding) => b.var, exp.bindings))));
    return bind(val, (vals: Value[]) => evalSequence(exp.body, makeExtEnv(vars, vals, env)));
}

const unevalValues = (vars:VarDecl[] ,res:Result<Value>):Result<Value[]> =>
    isOk(res) && isTupleValue(res.value) && res.value.vals.length === vars.length? makeOk(res.value.vals):
    makeFailure("Never");

const flat= (r:Result<Value[][]>):Result<Value[]> =>
    isOk(r)? makeOk(flatten(r.value)):
    makeFailure("Never")
...
const applyProcedure = (proc: Value, args: SExpValue[]): Result<Value> =>
